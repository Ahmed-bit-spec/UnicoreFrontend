      ...
      <div className="space-y-6 max-w-4xl mx-auto p-4 md:p-6 bg-gray-50 dark:bg-zinc-950 min-h-screen">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-gray-200 dark:border-zinc-800">
          <div className="text-center p-3">
            <p className={`text-3xl font-black ${isFailed ? "text-red-500" : "text-[#2C2DE0] dark:text-[#4F51FF]"}`}>
              {detail.percentage}%
            </p>
          ...
          <div className="text-center p-3 flex flex-col items-center justify-center">
            <span className={`px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest
              ${isFailed ? "bg-red-50 text-red-700 dark:bg-red-500/10 dark:text-red-400" : "bg-[#2C2DE0]/5 dark:bg-[#4F51FF]/10 text-[#0F0F55] dark:text-blue-300 dark:bg-[#2C2DE0] dark:bg-[#1E1FAA]/10 dark:text-[#4F51FF]"}`}
            >
          ...
          <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-white dark:bg-zinc-900 rounded-2xl max-w-md w-full border border-gray-200 dark:border-zinc-800 p-5 space-y-4">
              <div className="flex justify-between items-center">
                ...
                <button className="bg-[#2C2DE0] text-white text-sm font-bold shadow-[0_4px_0_#1E1FAA] hover:translate-y-0.5 hover:shadow-[0_2px_0_#1E1FAA] active:translate-y-1 active:shadow-none transition-all duration-150 group" onClick={() => setAppealingQId(null)} className="p-1 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded text-gray-400">✕</button>
              </div>
              <form onSubmit={handleAppealSubmit} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1.5">Explain your reason</label>
                  <textarea
                    required
                    rows={4}
                    value={appealReason}
            ...
            return (
              <div key={qid} className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800 p-5 space-y-4">
                ...
                {ans.correctAnswer !== undefined && (
                  <div>
                    <p className="text-xs font-bold uppercase tracking-wider text-[#2C2DE0] dark:text-[#4F51FF] mb-1">Correct Answer</p>
                    <div className="p-3 bg-[#2C2DE0]/5 dark:bg-[#4F51FF]/10/50 dark:bg-[#2C2DE0] dark:bg-[#1E1FAA]/5 border border-[#2C2DE0] dark:border-[#2C2DE0] dark:border-[#4F51FF]/10 rounded-xl text-sm text-[#0F0F55] dark:text-blue-300 dark:text-[#4F51FF]">
                  ...
                  {appeal ? (
                    <span className={`px-2.5 py-1 rounded text-xs font-bold uppercase tracking-wider
                      ${appeal.status === "pending" ? "bg-yellow-100 text-yellow-800" :
                        appeal.status === "modified" || appeal.status === "accepted" ? "bg-[#2C2DE0] text-[#0F0F55] dark:text-blue-200" :
    ...
    <div className="space-y-6 max-w-5xl mx-auto p-4 md:p-6 bg-gray-50 dark:bg-zinc-950 min-h-screen">
      <div>
        <h1 className="text-2xl font-black text-gray-900 dark:text-white flex items-center gap-2">
          <Award className="w-7 h-7 text-[#2C2DE0] dark:text-[#4F51FF]" />
          Exam Results
        </h1>
      ...
      {results.length === 0 ? (
        <div className="text-center py-20 bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800 text-gray-500 dark:text-gray-400">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {results.map((res) => {
            ...
            return (
              <div
                key={res._id}
                className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-gray-200 dark:border-zinc-800 hover:border-[#4F51FF] dark:border-blue-400 dark:hover:border-[#2C2DE0] dark:border-[#4F51FF]/30 transition-all flex flex-col justify-between gap-4"
              >
                <div className="space-y-2">
                  ...
                  <div className="flex items-end gap-3 pt-2">
                    ...
                    <div className="border-l border-gray-100 dark:border-zinc-800 pl-3">
                      <p className="text-2xl font-bold text-gray-500 dark:text-gray-400">{res.totalScore} / {res.totalMaxMarks}</p>
                ...
                >
                  <Play className="w-3 h-3 text-[#2C2DE0] dark:text-[#4F51FF] fill-current" /> View Detailed Feedback
                </button>
              </div>
            );
          })}
