...
const STATUS_DOT = {
  ...
  active: "bg-[#2C2DE0] dark:bg-[#1E1FAA]",
...
const StatCard = ({ label, value, color, icon: Icon, pulse, sub }) => (
  <div className="flex items-center gap-3 rounded-2xl border border-gray-200/70 bg-white/60 p-4 backdrop-blur-sm dark:border-white/10 dark:bg-white dark:bg-gray-900/[0.04]">
    <div className={`relative flex size-10 shrink-0 items-center justify-center rounded-xl ${color}`}>
      <Icon size={18} className="text-white" />
      {pulse && (
        <span className="absolute -right-0.5 -top-0.5 size-2.5 animate-pulse rounded-full bg-[#4F51FF] ring-2 ring-white dark:ring-gray-900" />
      )}
    </div>
    <div>
...
const DetailRow = ({ icon: Icon, label, value, mono }) => (
  <div className="flex items-start gap-3">
    <div className="mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-lg bg-gray-100 dark:bg-white dark:bg-gray-900/10">
      <Icon size={13} className="text-gray-500 dark:text-gray-400" />
    </div>
    <div className="min-w-0">
      <p className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 dark:text-gray-400">{label}</p>
  ...
  <div className="flex items-start gap-3">
    <div className="flex flex-col items-center">
      <div className={`flex size-6 shrink-0 items-center justify-center rounded-full text-[10px] font-black ring-2 ${done
        ? "bg-[#2C2DE0] dark:bg-[#1E1FAA] text-white ring-[#2C2DE0] dark:ring-[#2C2DE0]"
        : active
      ...
      {!last && (
        <div className={`mt-1 w-px flex-1 ${done ? "bg-[#2C2DE0] dark:bg-[#2C2DE0]" : "bg-gray-200 dark:bg-white dark:bg-gray-900/10"}`}
          style={{ height: 20 }} />
      )}
    </div>
    <div className="pb-3">
      <p className="text-xs font-semibold text-gray-700 dark:text-gray-300">{label}</p>
      {time && <p className="text-[10px] text-gray-400 dark:text-gray-500 dark:text-gray-400">{time}</p>}
    </div>
  </div>
);
  ...
  const columns = [
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-2.5">
          <div className="flex size-8 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#2C2DE0] dark:bg-[#1E1FAA]/10 text-[10px] font-black text-[#0F0F55] dark:text-blue-300 ring-1 ring-[#2C2DE0] dark:ring-[#4F51FF]/20 dark:text-[#4F51FF]">
            {row.user?.avatar
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-400 dark:text-gray-500 dark:text-gray-400 font-mono">
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-1.5">
          <Armchair size={13} className="text-[#2C2DE0] dark:text-[#4F51FF] flex-shrink-0" />
    ...
    },
    {
      ...
      render: (row) => (
        <div>
          ...
          <p className="text-[10px] text-gray-400 dark:text-gray-500 dark:text-gray-400">{fmtDate(row.startTime)}</p>
        </div>
      ),
    },
    {
  ...
  return (
    <PageTransition>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-5">
        ...
        <StatCard label={p.statsActive} value={stats.activeNow} color="bg-[#2C2DE0] dark:bg-[#1E1FAA]" icon={Radio} pulse sub={p.statsActiveSub} />
        ...
        renderRowActions={(row) => (
          <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
            <button className="bg-[#2C2DE0] text-white text-sm font-bold shadow-[0_4px_0_#1E1FAA] hover:translate-y-0.5 hover:shadow-[0_2px_0_#1E1FAA] active:translate-y-1 active:shadow-none transition-all duration-150 group"
              ...
              className="flex items-center gap-1 rounded-lg px-2 py-1.5 text-[10px] font-bold text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-white dark:bg-gray-900/10 transition-colors"
            >
              <Eye size={11} />
              {p.actionView}
            </button>
            ...
            {["completed", "cancelled", "no_show"].includes(row.status) && (
              <button className="bg-[#2C2DE0] text-white text-sm font-bold shadow-[0_4px_0_#1E1FAA] hover:translate-y-0.5 hover:shadow-[0_2px_0_#1E1FAA] active:translate-y-1 active:shadow-none transition-all duration-150 group"
                onClick={() => setViewingId(row._id || row.id)}
                className="rounded-lg px-2 py-1.5 text-[10px] font-bold text-gray-400 hover:bg-gray-100 dark:hover:bg-white dark:bg-gray-900/10 transition-colors"
      ...
      >
        {detailLoading ? (
          <div className="flex items-center justify-center py-12">
            <div className="size-6 animate-spin rounded-full border-2 border-[#2C2DE0] dark:border-[#4F51FF] border-t-transparent" />
          </div>
        ) : detailData ? (
          <div className="space-y-4">
            <div className="flex items-center gap-2.5 flex-wrap">
              ...
              <span className="text-[10px] text-gray-400 dark:text-gray-500 dark:text-gray-400 font-mono">#{detailData._id}</span>
            </div>

            <ConflictWarning conflicts={seatConflicts} title={p.conflictTitle} />

            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-3 rounded-2xl border border-gray-200/70 bg-white/50 p-4 dark:border-white/10 dark:bg-white dark:bg-gray-900/[0.03]">
                <p className="text-[10px] font-black uppercase tracking-wider text-gray-400 dark:text-gray-500 dark:text-gray-400">{p.sectionStudent}</p>
              ...
              <div className="space-y-3 rounded-2xl border border-gray-200/70 bg-white/50 p-4 dark:border-white/10 dark:bg-white dark:bg-gray-900/[0.03]">
                <p className="text-[10px] font-black uppercase tracking-wider text-gray-400 dark:text-gray-500 dark:text-gray-400">{p.sectionSeat}</p>
              ...
              <div className="space-y-3 rounded-2xl border border-gray-200/70 bg-white/50 p-4 dark:border-white/10 dark:bg-white dark:bg-gray-900/[0.03]">
                <p className="text-[10px] font-black uppercase tracking-wider text-gray-400 dark:text-gray-500 dark:text-gray-400">{p.sectionTime}</p>
              ...
              <div className="rounded-2xl border border-gray-200/70 bg-white/50 p-4 dark:border-white/10 dark:bg-white dark:bg-gray-900/[0.03]">
                <p className="mb-3 text-[10px] font-black uppercase tracking-wider text-gray-400 dark:text-gray-500 dark:text-gray-400">{p.sectionTimeline}</p>
            ...
            <div className="flex items-center gap-2 rounded-xl border border-gray-200/50 bg-gray-50/50 px-3 py-2 dark:border-white/5 dark:bg-white dark:bg-gray-900/[0.02]">
