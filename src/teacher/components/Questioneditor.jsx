...
function LinuxLabEditor({ question, index, onUpdate }) {
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-lg bg-[#2C2DE0]/5 dark:bg-[#4F51FF]/10 dark:bg-[#2C2DE0]/20 border border-[#2C2DE0] dark:border-[#2C2DE0] p-3 flex gap-3 items-start">
        <Terminal className="w-4 h-4 text-[#1E1FAA] dark:text-[#4F51FF] dark:text-[#4F51FF] mt-0.5 shrink-0" />
        <div className="text-xs text-[#0F0F55] dark:text-blue-300 dark:text-[#2C2DE0] space-y-1">
          <p>
            ...
            return (
              <label
                ...
                className={`flex items-center gap-2 p-2.5 rounded-lg border cursor-pointer transition-all select-none ${
                  checked
                    ? "bg-[#2C2DE0]/5 dark:bg-[#4F51FF]/10 border-[#4F51FF] dark:bg-[#2C2DE0]/30 dark:border-[#2C2DE0] shadow-sm"
      ...
      <div className="space-y-2">
        {testCases.map((tc, i) => (
          <div key={i} className="grid grid-cols-[1fr_1fr_auto_auto] gap-2 items-center">
            ...
            <button className="bg-[#2C2DE0] text-white text-sm font-bold shadow-[0_4px_0_#1E1FAA] hover:translate-y-0.5 hover:shadow-[0_2px_0_#1E1FAA] active:translate-y-1 active:shadow-none transition-all duration-150 group" type="button" onClick={() => removeTestCase(i)} className="text-red-400 hover:text-red-600 p-1">
        ...
        <div className="space-y-2">
          {(question.designResources || []).map((url, i) => (
             <div key={i} className="flex items-center gap-2">
                ...
                <button className="bg-[#2C2DE0] text-white text-sm font-bold shadow-[0_4px_0_#1E1FAA] hover:translate-y-0.5 hover:shadow-[0_2px_0_#1E1FAA] active:translate-y-1 active:shadow-none transition-all duration-150 group" type="button" onClick={() => {
...
export default function QuestionEditor({ question, index, onRemove, onUpdate, onUpdateOption }) {
  ...
  const typeColor = {
    ...
    lab:        "bg-[#2C2DE0] text-[#0F0F55] dark:text-blue-300 dark:bg-[#2C2DE0] dark:text-[#2C2DE0]",
    ...
    <div className="border rounded-xl mb-3 dark:border-neutral-700 overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-neutral-50 dark:bg-neutral-700/50">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button className="bg-[#2C2DE0] text-white text-sm font-bold shadow-[0_4px_0_#1E1FAA] hover:translate-y-0.5 hover:shadow-[0_2px_0_#1E1FAA] active:translate-y-1 active:shadow-none transition-all duration-150 group" onClick={() => setCollapsed(c => !c)} className="p-1 hover:bg-neutral-200 dark:hover:bg-neutral-600 rounded">
            {collapsed ? <ChevronDown className="w-4 h-4" /> : <ChevronUp className="w-4 h-4" />}
          </button>
          <button className="bg-[#2C2DE0] text-white text-sm font-bold shadow-[0_4px_0_#1E1FAA] hover:translate-y-0.5 hover:shadow-[0_2px_0_#1E1FAA] active:translate-y-1 active:shadow-none transition-all duration-150 group" onClick={() => onRemove(index)} className="p-1 hover:bg-red-100 dark:hover:bg-red-900/30 rounded text-red-500">
            <X className="w-4 h-4" />
          </button>
        </div>
      </div>
                ...
                <div className="flex gap-4">
                  {[true, false].map(val => (
                    <label key={String(val)} className="flex items-center gap-2 cursor-pointer">
                      ...
                      />
                      <span className={`text-sm font-medium ${val ? "text-[#1E1FAA] dark:text-[#4F51FF]" : "text-red-500"}`}>
