        ...
        return (
          <div key={idx} className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800 overflow-hidden">
            <div className="flex items-center gap-3 p-4 border-b border-gray-100 dark:border-zinc-800">
              <Code2 className="w-4 h-4 text-[#2C2DE0] dark:text-[#4F51FF]" />
              <div>
                <p className="text-sm font-bold text-gray-900 dark:text-white">
                  Q{idx + 1}: {ans.questionText?.substring(0, 80)}
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400">{ans.questionType || ans.type}</p>
              </div>
            </div>
            ...
            {(ans.codeOutput || ans.expectedOutput) && (
              <div className="grid grid-cols-1 sm:grid-cols-2 border-t border-gray-100 dark:border-zinc-800">
                ...
                {ans.expectedOutput && (
                  <div className="p-4">
                    <p className="text-xs font-bold text-[#2C2DE0] dark:text-[#4F51FF] mb-2 uppercase tracking-wider">Expected Output</p>
                    <pre className="text-xs font-mono text-gray-800 dark:text-gray-200 bg-[#2C2DE0]/5 dark:bg-[#4F51FF]/10 dark:bg-[#2C2DE0] dark:bg-[#1E1FAA]/5 p-3 rounded-lg overflow-auto max-h-32">
                      {ans.expectedOutput}
                    </pre>
                  </div>
                )}
